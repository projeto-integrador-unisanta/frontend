...
export function DeputadoModal({ politico, isOpen, onClose }: DeputadoModalProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-white p-6 md:p-8 rounded-2xl w-full max-w-[550px] shadow-2xl relative overflow-hidden text-[#002B5B]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-col md:flex-row gap-6 items-start">
          {/* CAIXA DESTACADA - Menor e mais compacta */}
          <div className="w-full md:w-40 flex-shrink-0">
            <div className="bg-gray-50 border border-gray-200 rounded-xl shadow-inner p-3 text-center">
              {politico.fotoUrl && (
                <img
          ...
          <div className="flex-1 space-y-3 w-full">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Estado</span>
                <span className="font-bold text-sm">{politico.estado || '-'}</span>
              </div>
              <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Partido</span>
                <span className="font-bold text-sm">{politico.partido || '-'}</span>
              </div>
            </div>

            <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
              <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">E-mail</span>
              <span className="font-medium text-xs break-all">{politico.email || 'N/A'}</span>
            </div>

            {/* SEÇÃO DE VOTOS */}
            <div className="pt-1">
              <div className="flex items-center justify-between gap-2 mb-2 border-b border-gray-100 pb-1.5">
                ...
                <input 
                  ...
                  className="text-[9px] bg-gray-100 border-none rounded-lg px-2 py-1 w-24 focus:ring-1 focus:ring-blue-500 outline-none font-bold uppercase"
                />
              </div>
        ...
        <button
          className="mt-6 w-full bg-gray-900 hover:bg-black text-white py-3 rounded-xl font-black uppercase tracking-widest text-[10px] transition-colors shadow-lg"
          onClick={onClose}
        >
          Fechar
        </button>
